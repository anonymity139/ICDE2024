...
/**
 * @brief Constructor
 * @param tps   Copy a same tuple set
 */
tuple_set::tuple_set(tuple_set *tps)
{
    relation_exist = NULL;
    bettupleset = NULL;
    int number_of_tuples = tps->tuples.size();
    int d_cat = tps->tuples[0]->d_cat, d_num = tps->tuples[0]->d_num;
    ...
    for (int i = 0; i < number_of_tuples; i++)
    {
        ...
        for (int j = 0; j < d_cat; j++)//categorical attributes
        {
            t->attr_cat[j] = tps->tuples[i]->attr_cat[j];
        }
        for (int j = 0; j < d_num; j++)//numerical attributes
        {
            t->attr_num[j] = tps->tuples[i]->attr_num[j];;
        }
        tuples.push_back(t);
    }
}
