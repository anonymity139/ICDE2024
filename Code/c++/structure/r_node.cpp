 ...
 * @param nd    R_node
 */
r_node::r_node(r_node *nd)
{
    ID = nd->ID;
    dim = nd->dim;
    len = nd->len;
    updated = nd->updated;
    is_modified = nd->is_modified;
    isRelationFound = nd->isRelationFound;
    s_1 = nd->s_1; s_2 = nd->s_2;


    for(int i = 0; i < nd->ineqleq.size(); ++i)
        ineqleq.push_back(nd->ineqleq[i]);
    for(int i = 0; i < nd->ineqgeq.size(); ++i)
        ineqgeq.push_back(nd->ineqgeq[i]);


    std::map<int, int> relationNode;
    copy(nd->relationNode.begin(), nd->relationNode.end(), inserter(relationNode, relationNode.begin()));

    /*
    if(nd->ext_leq != NULL)
    {
        int numext_leq = nd->ext_leq->size();
        if(numext_leq > 0)
        {
            ext_leq = new vector<point_t *>[nd->ext_leq->size()];
            for (int i = 0; i < nd->ext_leq->size(); ++i)
            {
                for (int j = 0; j < nd->ext_leq[i].size(); ++j)
                    ext_leq[i].push_back(nd->ext_leq[i][j]);
            }
        }
    }
    ...
    if(nd->ext_geq != NULL)
    {
        int numext_geq = nd->ext_geq->size();
        if(numext_geq > 0)
        {
            ext_geq = new vector<point_t *>[nd->ext_geq->size()];
            for (int i = 0; i < nd->ext_geq->size(); ++i)
            {
                for (int j = 0; j < nd->ext_geq[i].size(); ++j)
                    ext_geq[i].push_back(nd->ext_geq[i][j]);
            }
        }
    }
    */
    for(int i = 0; i < nd->relation.size(); ++i)
        relation.emplace_back(nd->relation[i].first, nd->relation[i].second);

    for(int i = 0; i < nd->sets.size(); ++i)
    {
        tuple_set *tt1 = new tuple_set(nd->sets[i].first);
        tuple_set *tt2 = new tuple_set(nd->sets[i].second);
        sets.emplace_back(tt1, tt2);
    }
}
...
void r_node::tuple_prune_all_rnode_aggre(hyperplane_set *R)
{
    ...
    if(size > 0)
    {
        ...
        for (int a = 0; a < size; ++a)//sets
        {
            if (sets[i].first == NULL || sets[i].second == NULL || sets[i].first->tuples.size() == 0 ||
                sets[i].second->tuples.size() == 0)
                sets.erase(sets.begin() + i);
            else
            {
                ...
                for (int j = 0; j < size1; ++j) //tuple 1
                {
                    ...
                    for (int k = 0; k < sets[i].second->tuples.size(); ++k) //tuple 2
                    {
                        ...
                        if (ineqleq.size() > 0 &&
                            R->dominated(ineqleq, ext_leq, v, -1, dom1))//R->R_dominate_leq(ineqleq, v))
                        {
                            sets[i].first->tuples.erase(sets[i].first->tuples.begin() + count1);
                            --count1;
                            break;
                ...
                for (int j = 0; j < size2; ++j) //tuple 1
                {
                    ...
                    for (int k = 0; k < sets[i].first->tuples.size(); ++k) //tuple 2
                    {
                        ...
                        if (ineqgeq.size() > 0 &&
                            R->dominated(ineqgeq, ext_geq, v, 1, dom2))//R->R_dominate_leq(ineqleq, v))
                        {
                            sets[i].second->tuples.erase(sets[i].second->tuples.begin() + count2);
                            --count2;
                            break;
...
void r_node::update_tupleset_relation(int direction)
{
    ...
    {
        for(int i = 0; i < sets.size(); ++i)
            if(sets[i].second->relation_exist != NULL)
                sets[i].second->relation_exist[sets[i].first->ID] = 1;
    }
    else
    {
        for(int i = 0; i < sets.size(); ++i)
            if(sets[i].first->relation_exist != NULL)
