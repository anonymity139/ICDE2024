 ...
 * @param initial   Signal: initialization
 */
hyperplane_set::hyperplane_set(hyperplane_set *hset)
{
    int dim = hset->hyperplanes[0]->d;
    if(dim > 1)
    {
        hyperplane *h;
        for (int i = 0; i < hset->hyperplanes.size(); ++i)
        {
            ...
            {
                if (j == i)
                    h->norm[j] = hset->hyperplanes[i]->norm[j];
                else
                    h->norm[j] = hset->hyperplanes[i]->norm[j];
            }
            h->offset = hset->hyperplanes[i]->offset;
            hyperplanes.push_back(h);
        }
    }
    //set extreme points
    //this->set_ext_pts(u_range);
    for (int i = 0; i < hset->ext_pts.size(); ++i)
    {
        point_t *p = new point_t(dim);
        for (int j = 0; j < dim; ++j)
            p->attr[j] = hset->ext_pts[i]->attr[j];
        ext_pts.push_back(p);
    }
}
...
int hyperplane_set::num_prune_same_cat(tuple_set *tset)
{
    int size = tset->tuples.size(), dim = hyperplanes[0]->d;
    int num_prune = 0;
    for(int i = 0; i < size; ++i)
